...
> **구현 반영 메모 (최종 코드)**  
> - UI 용어: **협력사** (구 목업의 「업체」)  
> - 저장소: Supabase Postgres (또는 env 미설정 시 로컬 JSON). Firebase Realtime 구독이 아님.  
> - 관리자 비밀번호: 서버 env `ADMIN_PASSWORD` (기본 `admin2026`).  
> - 데이터 API: `GET/PUT /api/dashboard` 전체 문서 덮어쓰기 모델.
...
서버 API(Supabase) 저장 후 재조회를 통해 여러 사용자의 변경사항이 반영된다. 다만 현재 구현은 문서 전체 덮어쓰기 방식이므로 동시 편집 시 마지막 저장 데이터가 이전 저장 데이터를 덮을 수 있다.
...
| 항목 | 내용 |
|---|---|
| 주요 목적 | 협력사별/참여자별 진척도 파악, 과제 관리, 피드백 작성, 레포트 발행 |
| 주요 화면 | 관리자 대시보드, 협력사 허브, 개인 대시보드, 레포트 모달 |
| 주요 권한 | 협력사 추가/삭제, 참여자 추가/삭제, 과제 추가/수정/삭제, 일정 편집, 강사 메모 작성, 레포트 발행 |
| 주요 리스크 | 관리자 비밀번호는 서버 `ADMIN_PASSWORD`로 검증(기본값 존재 — 배포 시 변경 권장) |
...
| 협력사 허브 | 가능 | 가능 | 협력사 단위 참여자 현황 확인 및 상세 이동 |
| 개인 대시보드 | 가능 | 가능 | 참여자별 과제/보고/피드백 관리 |
| 레포트 발행 모달 | 가능 | 불가 | 주간 레포트 스프레드시트 발행 |
| 일정 편집 모달 | 가능 | 불가 | 협력사별 프로젝트 일정 관리 |
...
| 상황 | 현재 의미 | 권장 안내 |
|---|---|---|
| 협력사 없음 | 운영 대상 협력사가 아직 등록되지 않음 | "등록된 협력사가 없습니다. 협력사를 추가해 프로젝트를 시작하세요." |
| 참여자 없음 | 특정 협력사에 참여자가 없음 | "아직 참여자가 없습니다. 참여자 등록 버튼으로 명단을 추가하세요." |
...
| 목표 | 전체 현황 관리 화면으로 진입 |
| 진입 조건 | 앱 첫 화면 접근 |
| 완료 조건 | 관리자 대시보드 또는 협력사 허브 접근 가능 |
...
- 관리자 비밀번호는 클라이언트 코드가 아닌 Supabase Authentication 기반으로 전환한다.
...
|---|---|
| 사용자 | 참여자 |
| 목표 | 본인이 속한 협력사 허브 또는 개인 대시보드로 진입 |
| 진입 조건 | 협력사에 참여자 데이터가 이미 등록되어 있음 |
...
2. 협력사를 선택한다.
3. 이름과 이메일을 입력한다.
4. 시스템은 선택한 협력사의 참여자 목록에서 이름/이메일 일치 항목을 찾는다.
5. 일치하는 참여자가 있으면 해당 참여자 권한으로 로그인한다.
6. 사용자는 협력사 허브에서 동료 현황을 보고, 본인 개인 대시보드로 이동할 수 있다.

#### 예외 흐름

| 상황 | 시스템 반응 | UX 권장 |
|---|---|---|
| 협력사 미선택 | 로그인 불가 | 협력사 선택 필드 강조 |
...
| 잘못된 협력사 선택 | 잘못된 협력사에 등록 가능 | 등록 전 협력사명 확인 문구 제공 |
...
|---|---|
| 사용자 | 관리자 |
| 목표 | 전체 협력사 수, 참여자 수, 평균 진척도를 빠르게 파악 |
| 진입 조건 | 관리자 로그인 완료 |
| 완료 조건 | 정체 참여자 또는 관리 대상 협력사를 식별 |

#### 기본 흐름

1. 관리자는 관리자 대시보드에 진입한다.
2. 상단 통계 카드에서 총 협력사 수, 총 참여자 수, 평균 진척도를 확인한다.
3. 협력사별 일정 관리 영역에서 각 협력사의 시작일, KickOff, 종료일을 확인한다.
4. 전사 실습 현황 테이블에서 모든 참여자의 협력사, 이름, 과제, 진척도, 상태, 강사 메모를 확인한다.
...
- 테이블은 협력사, 부서, 상태, 진척도 기준 필터가 필요하다.
- 강사 메모가 비어 있는 참여자를 빠르게 찾을 수 있어야 한다.

---

### 시나리오 E: 관리자가 협력사를 추가한다
...
| 목표 | 신규 프로젝트 협력사 등록 |
| 진입 조건 | 관리자 대시보드 접근 |
| 완료 조건 | 협력사 목록에 신규 협력사 표시 |

#### 기본 흐름

1. 관리자는 "협력사 추가" 버튼을 선택한다.
2. 협력사명 입력 모달이 열린다.
3. 협력사명을 입력하고 저장한다.
4. 시스템은 신규 협력사 객체를 생성한다.
5. 협력사 목록과 통계 카드가 갱신된다.

#### 예외 흐름

| 상황 | 시스템 반응 | UX 권장 |
|---|---|---|
| 협력사명 미입력 | 저장 불가 | 필수 입력 오류 표시 |
| 중복 협력사명 | 현재 명확한 방지 없음 | 중복 경고 및 계속 진행 여부 확인 |
| 저장 실패 | Supabase 반영 실패 | 실패 알림 및 재시도 제공 |

---

### 시나리오 F: 관리자가 협력사별 일정을 수정한다
...
| 진입 조건 | 협력사가 등록되어 있음 |
| 완료 조건 | 협력사별 일정이 갱신되고 개인 대시보드의 일정 대비 실적 계산에 반영 |

#### 기본 흐름

1. 관리자는 협력사별 일정 관리 영역에서 특정 협력사의 편집 버튼을 선택한다.
2. 일정 편집 모달이 열린다.
3. 시작일, KickOff 일자, 종료일을 입력한다.
4. 저장 시 해당 협력사의 `schedule` 값이 갱신된다.
...
### 시나리오 G: 사용자가 협력사별 참여자 현황을 확인한다

| 항목 | 내용 |
|---|---|
| 사용자 | 관리자, 참여자 |
| 목표 | 같은 협력사 참여자들의 부서별 현황과 진척도 확인 |
| 진입 조건 | 로그인 후 협력사 선택 상태 |
| 완료 조건 | 특정 참여자의 상세 현황으로 이동하거나 협력사 참여 현황을 확인 |

#### 기본 흐름

1. 사용자는 협력사 허브 화면에 진입한다.
...
1. 관리자는 관리자 대시보드 또는 협력사 허브에서 참여자 상세 화면에 진입한다.
...
| 진입 조건 | 협력사 일정이 설정되어 있음 |
| 완료 조건 | 양호/정상/정체 상태를 이해하고 필요한 조치를 판단 |

#### 기본 흐름

1. 시스템은 협력사의 시작일과 종료일을 기준으로 전체 기간을 계산한다.
...
### 시나리오 M: 관리자가 협력사별 주간 레포트를 발행한다

| 항목 | 내용 |
|---|---|
| 사용자 | 관리자 |
| 목표 | 협력사별 주간 진행 현황을 Google Sheets로 발행 |
| 진입 조건 | 협력사 허브 접근, 관리자 권한 |
| 완료 조건 | GAS Webhook으로 레포트 발행 요청 전송 |

#### 기본 흐름

1. 관리자는 협력사 허브에서 "레포트 발행" 버튼을 선택한다.
2. ReportModal이 열린다.
...
|---|---|
| 사용자 | 관리자 |
| 목표 | 현재 앱 내 협력사/부서/이름/이메일 정보를 외부 시트에 동기화 |
| 진입 조건 | 관리자 대시보드 접근 |
...
2. 시스템은 전체 협력사의 참여자 목록을 평탄화한다.
3. 협력사명, 부서, 이름, 이메일을 payload로 구성한다.
4. GAS URL로 `updateParticipants` 요청을 전송한다.
5. 사용자에게 요청 전송 결과를 안내한다.

#### UX 권장

- 업데이트 전 총 협력사 수와 총 참여자 수를 확인시킨다.
...
### 시나리오 O: 관리자가 협력사 또는 참여자를 삭제한다

| 항목 | 내용 |
|---|---|
| 사용자 | 관리자 |
| 목표 | 더 이상 운영하지 않는 협력사 또는 참여자 제거 |
| 진입 조건 | 관리자 권한 |
...
| 대상 | 삭제 영향 |
|---|---|
| 협력사 | 협력사 하위 참여자와 일정 데이터가 함께 사라짐 |
...
- 협력사 삭제는 가장 파급이 크므로 "협력사명 입력 후 삭제" 방식도 검토한다.
...
| 구분 | 요구사항 |
|---|---|
| 요약 통계 | 총 협력사 수, 총 참여자, 평균 진척도 표시 |
| 일정 관리 | 협력사별 시작일/KickOff/종료일 확인 및 편집 |
| 전체 테이블 | 협력사명, 참여자명, 과제, 진척도, 상태, 메모 표시 |
| 관리 액션 | 협력사 추가, 과제 관리, 참여자 업데이트 제공 |
...
- 기존 참여자는 협력사, 이름, 이메일로 로그인할 수 있다.
- 미등록 참여자는 부서를 입력해 신규 등록할 수 있다.
- 새로고침 시 인증 상태가 초기화되는 동작을 확인한다.

### 11.2 관리자 기능

- 관리자는 전체 협력사 수, 참여자 수, 평균 진척도를 확인할 수 있다.
- 관리자는 협력사를 추가하고 삭제할 수 있다.
- 관리자는 협력사별 일정을 수정할 수 있다.
